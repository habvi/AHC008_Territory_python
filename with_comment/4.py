...
# 通路(7, 15, 23)の人が独立して動物を塞ぐ
# yet : 人が同じマスにいる場合壁を置く予定のところに2人壁を置くみたいな同じ動きをする,エラーにはならないっぽいが
...
def print_debug1(g):
    print('> human_place          :', g.human_place)
    print('> top5_row_destination :', g.top5_row_destination)
    print('> human roles          :', g.roles)
    print('> top5_each_destination:', g.top5_each_destination)
    print('> number_of_each_aisle :', g.number_of_each_aisle)
    print('> who_each_aisle       :', g.who_each_aisle)
    print('> where_my_aisle       :', g.where_my_aisle)
    print('> done_roles           :', g.done_roles)
    print('> sorted_done_roles    :', sorted(g.done_roles))
    print('> done_roles_count     :', g.done_roles_count)
    print('> state_1 row          :', g.state_1)
    print('> state_2 col          :', g.state_2)
    print('> no_human             :', g.no_human)
...
def main():
    ...
    # print_debug1(g)
    with open('roles', 'a') as f:
        # print(set([g.m, sum(g.state_1), sum(g.state_2)]), g.roles, file=f)
        # print(g.roles, file=f)
        # print(g.human_place, file=f)
